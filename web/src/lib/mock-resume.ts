...
export const templateDemoResumes: Record<TemplateStyle, Resume> = {
  ...
  chinesePro: createResume({
    basicInfo: {
      ...
      location: '上海',
      website: '',
      github: 'github.com/limingyuan',
      linkedin: '',
    },
    summary:
      '10 年后端研发与架构经验，专注高并发分布式系统设计与数据平台建设。主导过日活千万级产品的后端架构升级，具备从技术选型、系统设计到团队管理的全链路经验。',
    experience: [
      {
        company: '上海某科技独角兽',
        position: '后端架构师 · 基础架构组负责人',
        startDate: '2022.03',
        endDate: '至今',
        location: '上海',
        description: [
          '主导核心交易系统微服务化改造，将单体应用拆分为 20+ 微服务，系统可用性从 99.5% 提升至 99.99%',
          '设计统一网关与消息中间件方案，支撑日均 10 亿级 API 调用，P99 延迟控制在 50ms 以内',
          '带领 8 人基础架构团队，建立技术评审与代码规范机制，推动团队技术能力提升',
        ],
      },
      {
        company: '杭州某电商平台',
        ...
        location: '杭州',
        description: [
          '负责搜索推荐服务架构设计与优化，重构索引构建流程，搜索响应时间从 200ms 优化至 40ms',
          '设计分布式缓存与数据库读写分离方案，支撑大促期间 QPS 峰值 50 万+',
          '推动 CI/CD 流水线建设与全链路压测体系落地，保障双 11 核心链路零故障',
        ],
      },
      {
        company: '北京某互联网金融公司',
        ...
        location: '北京',
        description: [
          '参与核心借贷系统后端研发，负责风控决策引擎与资金路由模块的设计与实现',
          '引入消息队列解耦业务链路，解决高并发场景下的订单积压问题，系统吞吐量提升 3 倍',
          '搭建自动化测试与监控告警平台，线上故障响应时间从 30 分钟缩短至 5 分钟',
        ],
      },
    ],
    education: [
      { school: '浙江大学', degree: '硕士', field: '计算机科学与技术', startDate: '2012', endDate: '2014' },
      { school: '华中科技大学', degree: '本科', field: '软件工程', startDate: '2008', endDate: '2012' },
    ],
    projects: [
      {
        name: '统一微服务网关平台',
        ...
        endDate: '2023.12',
        description: [
          '基于 Go 自研高性能 API 网关，支持动态路由、限流熔断、灰度发布、链路追踪等核心能力',
          '实现插件化架构设计，支持业务方自定义过滤规则，日均流量从 1 亿平稳扩展至 10 亿',
          '网关层 P99 延迟控制在 5ms 以内，通过公司技术委员会评审并推广至全公司',
        ],
      },
      {
        name: '实时数据同步平台',
        ...
        endDate: '2023.03',
        description: [
          '设计基于 CDC（Change Data Capture）的数据实时同步方案，支持 MySQL→ClickHouse→Redis 多链路',
          '实现断点续传与数据一致性校验机制，端到端延迟控制在秒级，数据准确率达 99.999%',
          '支撑 BI 报表、实时推荐、风控等 8 个业务线，日均同步数据量 200 亿行',
        ],
      },
      {
        name: '智能风控决策引擎',
        ...
        endDate: '2017.09',
        description: [
          '参与设计规则引擎 DSL 与可视化配置平台，支持运营人员零代码配置风控策略',
          '实现规则并行匹配与优先级排序算法，单次请求平均决策时间从 200ms 降至 15ms',
          '系统支撑日均 300 万笔借贷审核，坏账率降低 22%，获公司年度技术创新奖',
        ],
      },
    ],
    skills: [
    ...
    certifications: [
      { name: 'AWS Solutions Architect Professional', issuer: 'Amazon', date: '2022' },
      { name: 'Kubernetes Administrator (CKA)', issuer: 'CNCF', date: '2023' },
    ],
    languages: [
      { language: '中文', proficiency: '母语' },
      { language: '英文', proficiency: '流利（技术文档与会议）' },
    ],
  }),
